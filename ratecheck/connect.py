"""Native cognis-connect emit for ratecheck — forward findings to any platform.

Maps ratecheck's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    ratecheck ... --format json | ratecheck-emit --to stix
    ratecheck-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    """Tailor a ratecheck record toward the Finding contract before normalize().
