...
[![PyPI](https://img.shields.io/pypi/v/cognis-ratecheck.svg)](https://pypi.org/project/cognis-ratecheck/)
[![CI](https://github.com/cognis-digital/ratecheck/actions/workflows/ci.yml/badge.svg)](https://github.com/cognis-digital/ratecheck/actions)
...
RATECHECK exists for one job — probe api rate-limit/abuse resistance from a request spec — and does it without a SaaS bill or heavyweight setup.
...
```bash
ratecheck --version
ratecheck scan .                      # scan the current project
ratecheck scan . --format json
ratecheck scan . --fail-on high       # non-zero exit for CI gates
ratecheck mcp                         # expose as an MCP server (Cognis.Studio / Claude Desktop / Cursor)
```
...
`ratecheck` is one of the **100+ tools** in the [Cognis Neural Suite](https://github.com/cognis-digital).
